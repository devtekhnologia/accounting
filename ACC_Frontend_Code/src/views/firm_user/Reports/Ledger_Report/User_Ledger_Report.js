  ...
  useEffect(() => {
    const fetchTotalBalance = async () => {
      if (selectedFirmGl) {
        try {
          const gl_id = selectedFirmGl;
          const response = await fetch(`${api_url}/api/users/ledger_total_bal/${gl_id}`);
          if (!response.ok) {
                    ...
                    <CCol style={{ marginLeft: "11px" }} className='col-10'>
                      <CRow className='allfirms_icon_title_row align-items-center'>
                        ...
                        <CCol className='col-7 col-md-7'>
                          <h5 className="title_font mb-0" style={{ color: 'white' }}>Ledger Report</h5>
                        </CCol>
                      </CRow>
                    </CCol>
...
export default User_Ledger_Report;